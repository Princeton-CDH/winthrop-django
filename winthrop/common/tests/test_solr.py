...
@override_settings(SOLR_CONNECTIONS=TEST_SOLR_CONNECTIONS)
@patch('winthrop.common.solr.get_solr_connection')
...
class TestCoreAdmin(TestCase):
    ...
    @patch('winthrop.common.solr.requests')
...
class TestIndexable(TestCase):
    ...
    class SimpleIndexable(Indexable):
        # NOTE: simple indexable gets picked up as a subclass in
        # index test command; patch in mock objects methods to avoid errors
        objects = Mock()
        objects.count.return_value = 0
        objects.all.return_value = list()
